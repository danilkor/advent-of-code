...
int reducer(vector<uint16_t> &target, vector<set<uint16_t>> &buttons) {
    int res = 0;
    vector<uint16_t> cur = target;
    bool zero = false;
    while (!zero) {
        zero = true;
        bool any_gt_6 = false;
        ...
        // Find the smallest sum without zero
        set<uint16_t> smb;
        int smallest_sum = 0;
        for (auto btn : buttons) {
            ...
            if (cur_sum * smb.size() > smallest_sum * btn.size()) {
                smb = btn;
    ...
    return res;
    brootforce:
    vector<int> cur_seq = vector<int>(target.size(), 0);
    int biggest = 0;
    for (auto n : cur) {
        if (n > biggest) {
            biggest = n;
    ...
    int depth = 1;
    while (bf_res == 0) {
        bf_res = bruteforce2(cur, buttons, depth, 1, cur_seq);
        depth++;
    }
    res += bf_res;
...
int p2(vector<string>& lines) {
    ...
    for (string line: lines) {
        ...
        int res = reducer(target,buttons);
        sum += res;
